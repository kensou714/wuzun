...
function freshXunbaoData(player)
	if activity.isXunbaoDazhe() then
		player:set_panel_data(PANEL_ACTIONSET_NAME,"disCountBox","visible","true");
...
function xunbao(player)
	...
	local e = {"二级生命玄晶","二级攻击玄晶","三级生命玄晶","三级攻击玄晶","二级灵羽","三级灵羽","三级宝石","四级宝石","真气丹(500)","经验珠(200万)","祈福令牌(小)","转生石","进阶石包(100)","坐骑经验丹(大)","魂石令牌(1000)","完美的混天头盔"..job,"完美的混天项链"..job,"完美的混天护腕"..job,"完美的混天戒指"..job,"完美的混天腰带"..job,"完美的混天靴"..job,"混天镇元甲"..job,"混天镇元衣"..job,"承影剑"..job,"红色龙蛋"};
	...
	local l = {"五级生命玄晶","五级攻击玄晶","五级物防玄晶","五级魔防玄晶","七级灵羽","八级宝石","真气丹(10000)","经验珠(5000万)","神器积分[100分]","骠骑将军护腕","抚国大将军戒","镇国大将军链","兵马大元帅盔","完美的神皇头盔"..job,"完美的神皇项链"..job,"完美的神皇护腕"..job,"完美的神皇戒指"..job,"完美的神皇腰带"..job,"完美的神皇靴"..job,"神皇震威甲"..job,"神皇震威衣"..job,"神皇震威剑"..job,"三转螭龙盔"..job,"三转螭龙链"..job,"三转螭龙镯"..job,"三转螭龙戒"..job,"三转螭龙腰带"..job,"三转螭龙靴"..job,"三转螭龙甲"..job,"三转螭龙衣"..job,"三转螭龙剑"..job,"烈焰死神袍(永久)","烈焰死神衣(永久)"};
	local k = {"八级灵羽","八级宝石","九级宝石","十级宝石","黄色玫瑰","抚国大将军戒","镇国大将军链","兵马大元帅盔","神器积分[1000分]","龙皇离火剑·武威","龙皇离火剑·烈焰","龙皇离火剑·无极","四转洪荒盔"..job,"四转洪荒链"..job,"四转洪荒镯"..job,"四转洪荒戒"..job,"四转洪荒腰带"..job,"四转洪荒靴"..job,"四转洪荒甲"..job,"四转洪荒衣"..job,"四转洪荒剑"..job};
...
function process_lottery(player,actionset,panelid,actionid)
	...
	if data then
		if activity.isXunbaoDazhe() == false then
		...
		if(num>=data.times) then
			if activity.isXunbaoDazhe() then
			 	if player:get_vcoin() >= data.vcoin*0.9 then
					player:sub_vcoin(data.vcoin*0.9,"choujiang");
					for i=1,data.times do
						xunbao(player)
					end
				    if util.ppn(player,const.PP_JC_MRXBJL_TIME) ~= today() then
						player:set_param(const.PP_JC_MRXBJL_TIME,today());
						player:set_param(const.PP_JC_MRXBJL,data.times);
					else
						player:set_param(const.PP_JC_MRXBJL,util.ppn(player,const.PP_JC_MRXBJL)+data.times);
					end
					...
					player:alert(11,1,"您的元宝不足"..(data.vcoin*0.9)..",无法完成寻宝!");
				end
				return;
			end
