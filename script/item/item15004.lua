...
尊敬的玩家,
	...
	return 0;
		...
		player:set_status(7,3600,10000,1,11);
		player:alert(1,1,"您成功获得1小时双倍经验");
	end
end
...
function onTalkfg7(player)
	if player:num_item("双倍宝典",1) >= 1 then
		player:set_status(7,3600,10000,1,11);
		player:remove_item("双倍宝典",1);
		player:alert(1,1,"您成功获得1小时双倍经验");
	else
		player:alert(1,1,"对不起,您的背包没有双倍宝典,覆盖失败!");
	end
end