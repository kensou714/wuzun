...
local task_id=const.TASK_ID_FUBEN_SEXP;
local task_data={
	ttype="副本",		focus=true,		autogo=false,
	[1	]={
		tid=1,		nid=1,		need_level=45,		name="奴玛神庙",	accepter="副本使者",	doner="副本使者",
		need_type="mon",	need_target="",	need_map='rs%d',need_num=1,
		award={
	...
	[2	]={
		tid=2,		nid=2,		need_level=50,		name="猛犸神殿",	accepter="副本使者",	doner="副本使者",
		need_type="mon",	need_target="",	need_map='rs%d',need_num=1,
		award={
			{name="经验",		num=function(target,player) return math.min(80,player:get_level())*math.min(80,player:get_level())*363;end,},
			{name="绑定金币",	num=50000,},
		},
	},
	[3	]={
		tid=3,		nid=3,		need_level=55,		name="蛮牛大殿",	accepter="副本使者",	doner="副本使者",
		need_type="mon",	need_target="",	need_map='rs%d',need_num=1,
		award={
			{name="经验",		num=function(target,player) return math.min(80,player:get_level())*math.min(80,player:get_level())*569;end,},
			{name="绑定金币",	num=100000,},
		},
	},
	[4	]={
		tid=4,		nid=4,		need_level=60,		name="妖月巢穴",	accepter="副本使者",	doner="副本使者",
		need_type="mon",	need_target="",	need_map='rs%d',need_num=1,
		award={
			{name="经验",		num=function(target,player) return math.min(80,player:get_level())*math.min(80,player:get_level())*735;end,},
			{name="绑定金币",	num=100000,},
		},
	},
	[5	]={
		tid=5,		nid=5,		need_level=65,		name="巨灵魔窟",	accepter="副本使者",	doner="副本使者",
		need_type="mon",	need_target="",	need_map='rs%d',need_num=1,
		award={
			{name="经验",		num=function(target,player) return math.min(80,player:get_level())*math.min(80,player:get_level())*1102;end,},
			{name="绑定金币",	num=150000,},
		},
	},
	[6	]={
		tid=6,		nid=6,		need_level=70,		name="混沌魔域",	accepter="副本使者",	doner="副本使者",
		need_type="mon",	need_target="",	need_map='rs%d',need_num=1,
		award={
			{name="经验",		num=function(target,player) return math.min(80,player:get_level())*math.min(80,player:get_level())*1462;end,},
...
function check_done_talk(player,param)
	if nil == string.find(player:get_map():get_id(),"r%a%d") then
		return ;
	end
...
function give_award(player,data)
	if player:get_task_state(1101) == 0 then
		player:set_task_state(1101,1); player:push_task_data(1101,0);
	end
	for i=1,#data do
